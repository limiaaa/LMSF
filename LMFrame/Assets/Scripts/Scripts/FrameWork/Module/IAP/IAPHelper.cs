...
public class IAPHelper : MonoSingleton<IAPHelper>, IStoreListener
{
    ...
    (introductory_info_dict == null || !introductory_info_dict.ContainsKey(item.definition.storeSpecificId)) ? null : introductory_info_dict[item.definition.storeSpecificId];
        ...
        InitSuccess?.Invoke(m_ProductDic);
    }
    //延期
    ...
    public void OnPurchaseFailed(Product item, PurchaseFailureReason failureReason)
    {
        ...
        PayFailed?.Invoke(item, failureReason.ToString());
    }
    //购买成功后会调用此方法，需继续处理
...
#if UNITY_EDITOR
        PaySuccessInEditor?.Invoke(purchaseEvent.purchasedProduct);
    ...
    public void SavePendingPurchase(Product p)
    {
        ...
        PendingPurchase?.Invoke(p,confirmpendstate, m_Controller);
        Debug.Log("IAP_进行延时");
    }
    ...
    public void DoPurchase(string productID)
    {
        ...
        PayWaitCallBack?.Invoke();
        Debug.Log(m_Controller);
        Debug.Log(m_Controller.products);
        Debug.Log(m_Controller.products.WithID(productID));
        Debug.Log(MiniJson.JsonEncode("XiaoMai"));
    ...
    //设置初始化成功回调
    Action<Dictionary<string, Product>> InitSuccess;
    public void SetInitSuccessCallBack(Action<Dictionary<string, Product>> action)
    {
        InitSuccess = action;
    }
    //设置初始化失败回调
    Action<InitializationFailureReason> InitFailed;
    public void SetInitFailedCallBack(Action<InitializationFailureReason> action)
    {
        InitFailed = action;
    }
    //设置支付失败回调
    Action<Product, string> PayFailed;
    public void SetPayFailedCallBack(Action<Product, string> action)
    {
        PayFailed = action;
    }
    ...
    //设置支付失败回调
    Action<int> isPay;
    public void SetBuyCallBack(Action<int> action)
    {
        isPay = action;
    }
    //设置开始验证回调
    Action<Product> PaySuccessInEditor;
    public void SetPaySuccessInEditorCallBack(Action<Product> action)
    {
        PaySuccessInEditor = action;
    }
    //设置在编辑器下直接支付成功回调
    Action PayProcess;
    public void SetPayProcessCallBack(Action action)
    {
        PayProcess = action;
    }
    //设置订单验证回调
    Action<Product, PurchaseProcessingResult, IStoreController> PendingPurchase;
    public void SetPendingPurchaseCallBack(Action<Product, PurchaseProcessingResult, IStoreController> action)
    {
        PendingPurchase = action;
    }

    Action PayWaitCallBack;
    public void SetPayWaitCallBack(Action action)
    {
        PayWaitCallBack = action;
    }
