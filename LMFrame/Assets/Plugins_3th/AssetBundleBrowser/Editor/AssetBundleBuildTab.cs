...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        ToggleData m_BuildLua;
        ToggleData m_EncryptAssetBundle;
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            m_BuildLua = new ToggleData(false, "Build LUA", "Build LUA", m_UserData.m_OnToggles);
            ...
            m_EncryptAssetBundle = new ToggleData(false, "EncryptAssetBundle", "EncryptAssetBundle", m_UserData.m_OnToggles);
        ...
        internal void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(!AssetBundleModel.Model.DataSource.CanSpecifyBuildTarget))
            {
                ...
                if (tgt != m_UserData.m_BuildTarget)
                {
                    ...
                    if (m_UserData.m_UseDefaultPath)
                    {
                        ...
                        m_UserData.m_OutputPath += AppConst.MainAbName;//m_UserData.m_BuildTarget.ToString();
            ...
            using (new EditorGUI.DisabledScope(!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory))
            {
                ...
                newState = GUILayout.Toggle(m_BuildLua.state, m_BuildLua.content);
                if (newState != m_BuildLua.state)
                {
                    if (newState)
                        m_UserData.m_OnToggles.Add(m_BuildLua.content.text);
                    else
                        m_UserData.m_OnToggles.Remove(m_BuildLua.content.text);
                    m_BuildLua.state = newState;
                }
                ...
                newState = GUILayout.Toggle(
                    m_EncryptAssetBundle.state,
                    m_EncryptAssetBundle.content);
                if (newState != m_EncryptAssetBundle.state)
                {
                    if (newState)
                        m_UserData.m_OnToggles.Add(m_EncryptAssetBundle.content.text);
                    else
                        m_UserData.m_OnToggles.Remove(m_EncryptAssetBundle.content.text);
                    m_EncryptAssetBundle.state = newState;
                }
            }
        ...
        private void ExecuteBuild()
        {
            ...
            if (m_BuildLua.state)
            {
                BuildLuaAssetBundle.BuildLuaResource(m_UserData.m_OutputPath);
            }
            var _backupBundleMD5 = new Dictionary<string, string>();
            //生成MD5表
            CreateAssetBundleConfig.CreateConfig(m_UserData.m_OutputPath, _backupBundleMD5, _backupMD5File);
            if (m_EncryptAssetBundle.state)
            {
                if (File.Exists(_backupMD5File))
                {
                    var content = File.ReadAllText(_backupMD5File);
                    _backupBundleMD5 = LitJson.JsonMapper.ToObject<Dictionary<string, string>>(content);
                }

                AssetBundleEncrypt.IncrementalEncryptEachFileInThisFolder(m_UserData.m_OutputPath, _backupBundleMD5);
            }

            if (m_CopyToStreaming.state)
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath);
            EditorUtility.DisplayDialog(string.Empty, "整个打包完成!", "确定");
        }
        private static string _backupMD5File =>
            Path.Combine(Directory.GetCurrentDirectory(), "AssetBundles", "BackupMd5.txt");
