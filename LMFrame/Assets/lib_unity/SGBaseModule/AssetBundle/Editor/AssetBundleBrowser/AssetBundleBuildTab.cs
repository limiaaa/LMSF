...
using SG.AssetBundleBrowser.AssetBundleDataSource;

namespace SG.AssetBundleBrowser
{
    [System.Serializable]
    public class AssetBundleBuildTab:BaseTab
    {
        ...
        {
            if (m_ToggleData != null && m_ToggleData.Count > 0)
            {
                return;
            }
        ...
        internal void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope (!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)) {
                ...
                if (GUILayout.Button("Reset", GUILayout.MaxWidth(75f)))
                    ResetPathToDefault();
                if (GUILayout.Button("OpenFolder", GUILayout.MaxWidth(100f)))
                    OpenTargetFolder();
        ...
        private void ExecuteBuild()
        {
            ...
            if (AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory) {
                ...
                {
                    //string message = "active build target is diffrent with choose target.May be take a long time to build. are you sure to continue?";
                    string message = "当前平台不是目标平台,编译时间会大幅度增加.是否继续?";
                    if (EditorUtility.DisplayDialog("Build Target confirmation", message, "Yes", "No"))
                    {
                    }
                    else
                    {
                        EditorUtility.DisplayDialog("Cancel Build", "请修改Build Target.", "ok");
                        return;
                    }
                }
            ...
            buildInfo.onBuild = (assetBundleName) =>
            {
                ...
                EditorUtility.DisplayProgressBar("Building", assetBundleName, 0.5f);
            };
            EditorUtility.DisplayProgressBar("Building", "", 0);

            AssetBundleModel.Model.DataSource.BuildAssetBundles (buildInfo);
            EditorUtility.DisplayProgressBar("Building", "", 1.0f);
        ...
        private void OpenTargetFolder()
        {
            var gamePath = System.IO.Path.GetFullPath(".");
            gamePath = gamePath.Replace("\\", "/");
            gamePath += "/" + m_UserData.m_OutputPath;
            gamePath = gamePath.Replace("/", @"\");
            EditorUtility.RevealInFinder(gamePath);
        ...
        public class BuildTabData
        {
            ...
            internal string m_OutputPath = @"AssetBundles/StandaloneWindows/AssetBundle";
            internal bool m_UseDefaultPath = true;
            
            /// <summary>
            /// 0为live，1为test
            /// </summary>
            internal int m_Env = 1; // 0为live，1为test
            
            /// <summary>
            /// 0为english,1为chinese
            /// </summary>
            internal int m_Lang = 1; // 0为english,1为chinese

            internal string m_MainAppName;

            internal string m_SubAppName;

            internal string m_MainMinVersion;
