...
namespace SG.AssetBundleBrowser.AssetBundlePacker
{
    public class PackageDownloader : MonoBehaviour
    {
        ...
        IEnumerator StartDownloadPack()
        {
            ...
            var manifest = Common.LoadMainManifest();
            //var manifest = Common.LoadMainManifestBySubGame(packages_name_[0]);
            manifest.GetAllAssetBundles();

            ab_list.AddRange(manifest.GetAllAssetBundles());
            if (ab_list == null)
            {
            ...
            ab_download_.Start(Common.PATH +"/" + packages_name_[0], ab_list, resources_manifest);
