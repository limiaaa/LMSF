...
namespace SG
{
    public class RuntimeDebug : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (CommandPositions==null || CommandPositions.Length<=0)
            {
                return;
            }
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    DebugUtils.Log("Passed: " + name + "     " + CommandPositions[currentInx] + "    <----" + currentInx);
                    currentInx++;
                }
                else
                {
                    // DebugUtils.Log("failed: " + CommandPositions[currentInx] + "    <----" + currentInx);
