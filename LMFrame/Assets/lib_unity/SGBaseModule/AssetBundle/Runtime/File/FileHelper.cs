...
namespace SG.AssetBundleBrowser
{
    ...
    public static class FileHelper
    {
        ...
        public static uint GetFileCrc(string file_path)
        {
            long sum = 0;
            if (!File.Exists(file_path))
            {
            ...
            else
            {
                ...
                return (uint) CryptUtils.CRC32(bytes);
            }
        }
            ...
            if (src.Contains("://"))
            {
                ...
                if (string.IsNullOrEmpty(www.error))
                {
                    while (www.isDone == false)
                        yield return null;
                    AssetBundleBrowser.FileHelper.WriteBytesToFile(dest, www.downloadHandler.data,
                        www.downloadHandler.data.Length);
                    DebugUtils.Log("CopyStreamingAssetsToFile:>>src:{0}--des:{1},data:{2}" , src , dest , www.downloadHandler.data.Length);
                }
                else
                {
        ...
        public static void SyncCopyStreamingAssetsToFile(string src, string dest)
        {
#if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_ANDROID
            if (!src.Contains("///"))
                src = "file:///" + src;
            ...
            if (src.Contains("://"))
            {
                ...
                {
                    Debug.LogError(www.error + ":" + www.url + " src:" + src + " dest:" + dest);
                }
            }
            else
            {
                ...
                {
                    var data = File.ReadAllBytes(src);
                    FileHelper.WriteBytesToFile(dest, data, data.Length);
                }
                else
                {
                    Debug.LogError("Not exist: " + src);
        ...
        public static void ConfirmDirectory(string dir)
        {
